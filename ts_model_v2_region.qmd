...
#Prepare combined test cohort
  ...
  #Remove variabels for data dist for rms package
  md.test.all.cc <- md.test.all.cc %>% dplyr::select(-timeprevcombo_less61,-hb_extreme_53,-multi_drug_start,-INS.c)

#Prepare discontinuation dataset  
  load(paste0(data_dir,"disc.2004.Rda")) #Training set

  #Drop those age <80
  n.p <- nrow(disc.2004)
  disc.2004 <- disc.2004 %>% filter(agetx<80)
  c1 <- n.p-nrow(disc.2004)

  #Recast ncurrtx
  disc.2004 <- disc.2004 %>% mutate(ncurrtx=fct_collapse(ncurrtx,"4+"=c("4","5+")))
  levels(disc.2004$ncurrtx)
  
  #Define complete case set for variables with missing data
  disc.2004.cc <- disc.2004 %>% 
    filter(complete.cases(pretotalcholesterol,
                          prehdl,
                          prealt,
                          preegfr,
                          prebmi)) %>%
    mutate(hba1cmonth=12)
  #Remove variabels for data dist for rms package
  disc.2004.cc <- disc.2004.cc %>% dplyr::select(-timeprevcombo_less61,-hb_extreme_53,-multi_drug_start,-INS.c)
...
#Set fail dataset
  ...
  fail.data <- disc.2004.cc
  fail.data <- disc.2004.cc %>% dplyr::filter(prehba1c<76)
  fail.data <- disc.2004.cc %>% dplyr::filter(drugclass!="DPP4")
...
prop.table(table(fail.data$concordant))
